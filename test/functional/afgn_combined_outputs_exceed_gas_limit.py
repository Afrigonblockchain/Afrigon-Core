...
class AfgnCombinedOutputsExceedGasLimitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx = CTransaction()
        tx.vin = [make_vin(self.node, int(4000*COIN))]
        tx.vout = [
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(AFGN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(AFGN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(AFGN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL]))
        ]
...
if __name__ == '__main__':
    AfgnCombinedOutputsExceedGasLimitTest().main()

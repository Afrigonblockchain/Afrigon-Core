...
from .afgnconfig import COINBASE_MATURITY
from .afgn import generatesynchronized
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def setup(self):
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/afgnd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/afgn-cli' + config["environment"]["EXEEXT"])
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            for entry in os.listdir(cache_path()):
                if entry not in ['chainstate', 'blocks', 'stateAfgn']:  # Only keep chainstate and blocks folder

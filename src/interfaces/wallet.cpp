...
public:
    ...
    bool isUnspentAddress(const std::string &afgnAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == afgnAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_afgnDelegation.ExistDelegationContract() ? m_afgnDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_afgnDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_afgnDelegation.ExistDelegationContract() ? m_afgnDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_afgnDelegation.VerifyDelegation(address, delegation);
    ...
    AfgnDelegation m_afgnDelegation;
};

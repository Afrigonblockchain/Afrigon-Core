...
const char * const BITCOIN_CONF_FILENAME = "afgn.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Afgn
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Afgn
    // Mac: ~/Library/Application Support/Afgn
    // Unix: ~/.afgn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Afgn";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Afgn";
#else
    // Unix
    return pathRet / ".afgn";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Afrigon Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Afrigon Core Developers";
    }

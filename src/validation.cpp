...
#if defined(NDEBUG)
# error "Afgn cannot be compiled without assertions."
#endif
...
std::unique_ptr<AfgnState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = afgnDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = afgnDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        AfgnTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractAfgnTX resultConverter;
        if(!converter.extractionAfgnTransactions(resultConverter)){
        ...
        std::vector<AfgnTransaction> afgnTransactions = resultConverter.first;
        std::vector<EthTransactionParams> afgnETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(AfgnTransaction afgnTransaction : afgnTransactions){
            sumGas += afgnTransaction.gas() * afgnTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, afgnTransaction.gasPrice());
            } else {
                txMinGasPrice = afgnTransaction.gasPrice();
            }
            VersionVM v = afgnTransaction.getVersion();
            if(v.format!=0)
            ...
            if(afgnTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(afgnTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(afgnTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)afgnTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AFGN additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // afgn
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // afgn
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    AfgnTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = AfgnTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = AfgnTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<AfgnTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path afgnDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(afgnDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(afgnDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(AfgnTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, AfgnTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool AfgnTxConverter::extractionAfgnTransactions(ExtractAfgnTX& afgntx){
    ...
    std::vector<AfgnTransaction> resultTX;
    ...
    afgntx = std::make_pair(resultTX, resultETP);
    return true;
}

bool AfgnTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool AfgnTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AfgnTransaction AfgnTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AfgnTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AfgnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AfgnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t AfgnTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // afgn
    AfgnDGP afgnDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setAfgnSchedule(afgnDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = afgnDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = afgnDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = afgnDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // afgn
...
///////////////////////////////////////////////////////////////////////////////////////// afgn
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            AfgnTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractAfgnTX resultConvertAfgnTX;
            if(!convert.extractionAfgnTransactions(resultConvertAfgnTX)){
            ...
            if(!CheckMinGasPrice(resultConvertAfgnTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertAfgnTX.first, blockGasLimit, pindex->pprev);
            ...
            for(AfgnTransaction& qtx : resultConvertAfgnTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertAfgnTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertAfgnTX.first[k].from(),
                        resultConvertAfgnTX.first[k].to(),
                        ...
                        resultConvertAfgnTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(20000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // afgn
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // afgn
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // afgn
            globalState->setRootUTXO(oldHashUTXORoot); // afgn
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // afgn
        uint32_t sizeBlockDGP = afgnDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Afgn PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        AfgnTxConverter convert(tx, NULL, NULL, contractflags);

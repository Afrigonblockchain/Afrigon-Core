...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // afgn
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // afgn

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // afgn
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // afgn
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // afgn
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // afgn
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if afgnd is started with -deprecatedrpc=size\n"
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The afgn address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    AfgnDelegation afgnDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!afgnDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = afgnDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The afgn address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!afgnDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = afgnDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of afgn addresses",
                                    {{RPCResult::Type::STR, "address", "afgn address"}}},
                            }},
...
static UniValue arc20balanceof(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20balanceof",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO,  "The afgn address to check token balance"},
                },
                RPCResult{
...
static UniValue arc20allowance(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20allowance",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"addressFrom", RPCArg::Type::STR, RPCArg::Optional::NO,  "The afgn address of the account owning tokens"},
                    {"addressTo", RPCArg::Type::STR, RPCArg::Optional::NO,  "The afgn address of the account able to transfer the tokens"},
                },
                RPCResult{
...
static UniValue arc20listtransactions(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20listtransactions",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"addresss", RPCArg::Type::STR, RPCArg::Optional::NO,  "The afgn address to get history for."},
            ...
            RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::STR, "receiver", "The receiver afgn address"},
                            {RPCResult::Type::STR, "sender", "The sender afgn address"},

...
UniValue transactionReceiptToJSON(const AfgnTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination afgnSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(afgnSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&afgnSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToAfgnAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToAfgnAddress(tokenEvent.receiver, tokenEvent.receiver);
            }

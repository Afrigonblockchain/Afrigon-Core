 ...
 */
static const char* BITCOIN_PID_FILENAME = "afgnd.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("afgn-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/afrigonblockchain/afrigon-core>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateAfgn");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path afgnStateDir = GetDataDir() / "stateAfgn";
                bool fStatus = fs::exists(afgnStateDir);
                const std::string dirAfgn(afgnStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAfgnstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AfgnState>(new AfgnState(dev::u256(0), AfgnState::openDB(dirAfgn, hashDB, dev::WithExisting::Trust), dirAfgn, existsAfgnstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(afgnStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setAfgnSchedule(afgnDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));

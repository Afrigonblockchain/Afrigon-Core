...
#include <qt/afgnversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    afgnVersionChecker = new AfgnVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Afrigon address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and afgn: URIs)"));
    ...
    ARCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/arctoken"), tr("&ARC Tokens"), this);
    ARCTokenAction->setStatusTip(tr("ARC Tokens (send, receive or add Tokens in list)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Afrigon addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Afrigon addresses"));
    ...
    openAction->setStatusTip(tr("Open a afgn: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Afgn command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && afgnVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(AFGN_RELEASES, AFGN_RELEASES);
            QString message(tr("New version of Afgn wallet is available on the Afgn source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Afgn network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {

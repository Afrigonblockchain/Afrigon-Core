...
Token::Token()
{
    ...
    lstMandatory.append(AfgnToken::paramAddress());
    lstMandatory.append(AfgnToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(AfgnToken::paramSender());
    ...
    lstMandatory.append(AfgnToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(AfgnToken::paramAmount());
    lstOptional.append(AfgnToken::paramGasLimit());
    lstOptional.append(AfgnToken::paramGasPrice());
    lstOptional.append(AfgnToken::paramSender());
    lstOptional.append(AfgnToken::paramBroadcast());
    lstOptional.append(AfgnToken::paramChangeToSender());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToAfgnAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToAfgnAddress(tokenEvent.receiver, tokenEvent.receiver);
            }

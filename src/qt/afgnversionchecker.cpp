...
#define paternVersion "afgn-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

AfgnVersionChecker::AfgnVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

AfgnVersionChecker::~AfgnVersionChecker()
{

}

bool AfgnVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> AfgnVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(AFGN_RELEASES)));
    QEventLoop event;
...
Version AfgnVersionChecker::getMaxReleaseVersion()
{

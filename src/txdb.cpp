    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // afgn
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // afgn
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSigDlgt    = diskindex.vchBlockSigDlgt; // afgn

...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setAfgnSchedule(afgnDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = afgnDGP.getBlockSize(nHeight);
    minGasPrice = afgnDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = afgnDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    AfgnTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractAfgnTX resultConverter;
    if(!convert.extractionAfgnTransactions(resultConverter)){
    ...
    for(AfgnTransaction afgnTransaction : afgnTransactions){
        ...
        if(bceResult.usedGas + afgnTransaction.gas() > softBlockGasLimit){
        ...
        if(afgnTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, afgnTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            afgnDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = afgnDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                afgnDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                afgnDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            afgnDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            afgnDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    AfgnDelegation afgnDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                afgnDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = afgnDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    afgnDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    afgnDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                afgnDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(afgnDelegations.GetDelegation(address, delegation) && AfgnDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "afgnstake";
...
void StakeAfgns(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{

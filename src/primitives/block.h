...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // afgn
    uint256 hashUTXORoot; // afgn
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // afgn
        READWRITE(hashUTXORoot); // afgn
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // afgn
        hashUTXORoot.SetNull(); // afgn
    ...
    virtual bool IsProofOfStake() const //afgn
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //afgn
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //afgn
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // afgn
        block.hashUTXORoot   = hashUTXORoot; // afgn

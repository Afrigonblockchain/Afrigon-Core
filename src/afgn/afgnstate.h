...
#include <afgn/afgntransaction.h>
...
class AfgnTransactionReceipt: public dev::eth::TransactionReceipt {
public:
    AfgnTransactionReceipt(dev::h256 const& state_root, dev::h256 const& utxo_root, dev::u256 const& gas_used, dev::eth::LogEntries const& log) : dev::eth::TransactionReceipt(state_root, gas_used, log), m_utxoRoot(utxo_root) {}
...
class AfgnState : public dev::eth::State {
    
public:

    AfgnState();

    AfgnState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, AfgnTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createAfgnAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<AfgnState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<AfgnState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(AfgnState* _state, const std::vector<TransferInfo>& _transfers, const AfgnTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const AfgnTransaction& transaction;

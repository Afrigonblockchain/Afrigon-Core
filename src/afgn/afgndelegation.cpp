...
bool AfgnDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool AfgnDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool AfgnDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> AfgnDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void AfgnDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool AfgnDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string AfgnDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool AfgnDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{

...
void AfgnDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool AfgnDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule AfgnDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t AfgnDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AfgnDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AfgnDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AfgnDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AfgnDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AfgnDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AfgnDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AfgnDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AfgnDGP::createParamsInstance(){
...
dev::Address AfgnDGP::getAddressForBlock(unsigned int blockHeight){
...
void AfgnDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AfgnDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AfgnDGP::parseStorageOneUint64(uint64_t& value){
...
void AfgnDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AfgnDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){

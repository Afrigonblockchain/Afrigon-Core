...
//    initState();
    ...
    txs.push_back(createAfgnTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createAfgnTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createAfgnAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<AfgnTransaction> txsCall;
    txsCall.push_back(createAfgnTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<AfgnTransaction> txsMint;
    txsMint.push_back(createAfgnTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<AfgnTransaction> txsbalance;
    txsbalance.push_back(createAfgnTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<AfgnTransaction> txs;
    txs.push_back(createAfgnTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createAfgnAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<AfgnTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createAfgnTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));

...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createAfgnTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createAfgnTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createAfgnTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createAfgnTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    initState();
    contractLoading();
    AfgnDGP afgnDGP(globalState.get());
    dev::eth::EVMSchedule schedule = afgnDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    AfgnDGP afgnDGP(globalState.get());
    dev::eth::EVMSchedule schedule = afgnDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    dev::eth::EVMSchedule schedule = afgnDGP.getGasSchedule(coinbaseMaturity + 900);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AfgnTransaction> txs;
    txs.push_back(createAfgnTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createAfgnTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    AfgnDGP afgnDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    dev::eth::EVMSchedule schedule = afgnDGP.getGasSchedule(coinbaseMaturity + 2); // After initializing the tests, the height of the chain 502
...
//    initState();
    ...
    for(size_t i = 0; i < sizeList; i++){
        dev::eth::EVMSchedule schedule = afgnDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    uint32_t blockSize = afgnDGP.getBlockSize(nHeight);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    uint32_t nHeight = 0;
    uint32_t blocktimeDownscaleFactor = Params().GetConsensus().BlocktimeDownscaleFactor(nHeight);
    uint32_t blockSize = afgnDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP / blocktimeDownscaleFactor);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AfgnTransaction> txs;
    txs.push_back(createAfgnTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createAfgnTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    AfgnDGP afgnDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint32_t blockSize = afgnDGP.getBlockSize(coinbaseMaturity + 2);
...
//    initState();
    ...
    for(size_t i = 0; i < sizeList; i++){
        ...
        uint32_t blockSize = afgnDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    AfgnDGP afgnDGP(globalState.get());
    uint64_t minGasPrice = afgnDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    AfgnDGP afgnDGP(globalState.get());
    uint64_t minGasPrice = afgnDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AfgnTransaction> txs;
    txs.push_back(createAfgnTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createAfgnTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAfgnTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    AfgnDGP afgnDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint64_t minGasPrice = afgnDGP.getMinGasPrice(coinbaseMaturity + 2);
...
//    initState();
    ...
    size_t sizeList = Params().GetConsensus().CoinbaseMaturity(0) + 800;
    for(size_t i = 0; i < sizeList; i++){
        uint64_t minGasPrice = afgnDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;

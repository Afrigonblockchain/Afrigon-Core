...
void checkResult(bool isCreation, std::vector<AfgnTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AfgnTxConverter converter(transaction, NULL);
    ExtractAfgnTX afgnTx;
    BOOST_CHECK(converter.extractionAfgnTransactions(afgnTx));
    std::vector<AfgnTransaction> result = afgnTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(afgntxconverter_tests, TestingSetup)

...
extern std::unique_ptr<AfgnState> globalState;
...
inline void initState(){
    ...
    const std::string dirAfgn = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AfgnState>(new AfgnState(dev::u256(0), AfgnState::openDB(dirAfgn, hashDB, dev::WithExisting::Trust), dirAfgn + "/afgnDB", dev::eth::BaseState::Empty));
...
inline dev::Address createAfgnAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline AfgnTransaction createAfgnTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    AfgnTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AfgnTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AfgnTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<AfgnTransaction> txs){

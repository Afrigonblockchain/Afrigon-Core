Gitian building
================
*Setup instructions for a Gitian build of Afrigon Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Afgn
...
- [Building Afrigon Core](#building-afgn-core)
...
Clone the git repositories for afgn and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/afrigonblockchain/afrigon-core --recursive
git clone https://github.com/afgnproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/afgn/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the afgn repository under 'Fetch and create inputs' to install sources which require
...
To build Afrigon Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the afgn repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/afgn/.git/
...
For example:
```bash
URL=https://github.com/laanwj/afgn.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit afgn=${COMMIT} --url afgn=${URL} ../afgn/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit afgn=${COMMIT} --url afgn=${URL} ../afgn/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit afgn=${COMMIT} --url afgn=${URL} ../afgn/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-afgn:
./bin/gbuild --commit afgn=${COMMIT},cpp-eth-afgn=${ETHCOMMIT} --url afgn=${URL},cpp-eth-afgn=${ETHURL} ../afgn/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the afgn git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../afgn/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/afrigonblockchain/afrigon-core-detached-sigs.git

BTCPATH=/some/root/path/afgn
SIGPATH=/some/root/path/afgn-detached-sigs

./bin/gbuild --url afgn=${BTCPATH},signature=${SIGPATH} ../afgn/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/afgn-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/afgn-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/afgn-osx-build.assert
```
...
[afgnproject/gitian.sigs](https://github.com/afgnproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@afgn.org) and he will commit them.
